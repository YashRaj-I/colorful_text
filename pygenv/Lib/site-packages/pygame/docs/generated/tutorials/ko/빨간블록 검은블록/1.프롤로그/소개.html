...
<html lang="en" data-content_root="../../../../">
  <head>
    ...
    <link rel="next" title="Author: Youngwook Kim (Korean)" href="../2.%ED%85%8D%EC%8A%A4%ED%8A%B8%20%EC%B6%9C%EB%A0%A5/%EA%B8%B0%EC%B4%88%20%ED%85%9C%ED%94%8C%EB%A6%BF%EA%B3%BC%20%EC%B6%9C%EB%A0%A5.html" />
    <link rel="prev" title="Author: Youngwook Kim (Korean)" href="../../../en/Red_or_Black/8.Epilog/Epilog.html" /> 
  </head><body>  
...
<section id="id2">
<p>게임도 프로그램의 일부이기 때문에, 게임은 입력, 처리 그리고 출력으로 구성된다. C 콘솔 환경에서 게임을 만든다고 가정해 보자(C로 소스코드를 작성한 후 콘솔에서 실행하는 방식). 그러면 입력은 수많은 scanf(또는 엔터 입력 필요 없는 비표준 getch) 함수로, 처리는 항상 절차적으로 실행되는 복잡한 알고리즘으로, 출력은 아스키아트를 이용한 printf(그리고 화면을 지우는 깜빡거리는 clear)함수로 구성할 수 있다. 하지만 이렇게 만든 게임은 구식이고, 그래픽 없는 CUI고, 끉겨 보인다는 단점이 있다. 이런 식의 게임을 만드는 것이 질린다면, 보통 Unity 게임엔진이나 Unreal 게임엔진 등 게임 엔진에도 손을 대보게 된다. 하지만, 게임 엔진은 입문장벽이 높다는 단점이 있다. 복소수좌표를 활용한 공간상에서의 충돌, Mechanin/Legacy 애니메이션 호환성, 더 좋은 그래픽을 위한 더 큰 메모리/더 빠른 CPU… 아무래도 콘솔 환경과 게임 엔진 사이에는 딜레마가 있는 것 같다. 이 딜레마를 해결할 수 있을까?</p>
...
<p>(파이게임 게임의 예시- 배틀십)</p>
<p>요약하자면, 파이게임은 저급 수준(콘솔 환경은 예시 중 하나)의 게임 제작 프로그램과 고급 수준(게임 엔진은 예시 중 하나)의 게임 제작 프로그램의 장점을 모두 가진다는 것이다. 파이게임은 이 둘 사이의 좋은 연결점이 된다. 이것이 파이게임을 쓸 이유이다. 더 복잡한 게임 엔진을 최대한 활용해 게임을 만드는 1인 개발자가 목표가 아닌 이상 (빨리 그 게임 엔진을 배우는 것이 낫다!), 콘솔 환경용 게임이 아닌 더 발전된 환경에서 게임을 한번쯤은 코딩해 보고 싶다면 (물론, 푹 빠지면 계속 코딩하게 될 것이다!), 한번쯤은 파이게임을 시도해 볼만 하다.</p>
</section>


<br /><br />
<hr />
<a href="https://github.com/pygame-community/pygame-ce/edit/main/docs/reST/tutorials\ko\빨간블록 검은블록\1.프롤로그\소개.rst" rel="nofollow">Edit on GitHub</a>
    ...
    <div class="related" role="navigation" aria-label="Related">
      ...
      <ul>
        ...
        <li class="right" >
          <a href="../../../en/Red_or_Black/8.Epilog/Epilog.html" title="Author: Youngwook Kim (Korean)"
